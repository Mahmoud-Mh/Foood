...
export class RecipesController {
  ...
  @ApiQuery({ name: 'difficulty', required: false, enum: DifficultyLevel, description: 'Filter by difficulty' })
  @ApiQuery({ name: 'maxPrepTime', required: false, type: Number, description: 'Maximum prep time in minutes' })
  @ApiQuery({ name: 'maxCookTime', required: false, type: Number, description: 'Maximum cook time in minutes' })
  @ApiQuery({ name: 'minServings', required: false, type: Number, description: 'Minimum servings' })
  @ApiQuery({ name: 'maxServings', required: false, type: Number, description: 'Maximum servings' })
  ...
  async findPublished(
    ...
    return ApiResponseDto.success('Published recipes retrieved successfully', result);
  }
  ...
  async findFeatured(
    ...
    return ApiResponseDto.success('Featured recipes retrieved successfully', result);
  }
  ...
  async search(
    ...
    const result = await this.recipesService.search(searchTerm, paginationDto, filters);
    return ApiResponseDto.success('Search results retrieved successfully', result);
  }
  ...
  async findByCategory(
    ...
    const result = await this.recipesService.findAll(paginationDto, filters);
    return ApiResponseDto.success('Category recipes retrieved successfully', result);
  }
  ...
  @ApiParam({ name: 'difficulty', enum: DifficultyLevel, description: 'Recipe difficulty level' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10)' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
  ...
  async findByDifficulty(
    @Param('difficulty', new ParseEnumPipe(DifficultyLevel)) difficulty: DifficultyLevel,
    @Query('page', new DefaultValuePipe(1), ParseIntPipe) page: number,
    @Query('limit', new DefaultValuePipe(10), ParseIntPipe) limit: number,
  ): Promise<ApiResponseDto<PaginatedResultDto<RecipeListResponseDto>>> {
    const paginationDto: PaginationDto = { page, limit };
    ...
    const result = await this.recipesService.findAll(paginationDto, filters);
    return ApiResponseDto.success('Difficulty recipes retrieved successfully', result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Retrieve paginated list of recipes with admin access to all statuses'
  })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10)' })
  @ApiQuery({ name: 'status', required: false, enum: RecipeStatus, description: 'Filter by status (admin only)' })
  @ApiResponse({ 
    status: HttpStatus.OK, 
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid recipe data' })
  async create(
  ...
  @ApiOperation({
    ...
    description: 'Retrieve recipes created by the current user (including drafts)'
  })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10)' })
  @ApiResponse({ 
  ...
  async findMyRecipes(
    ...
    const paginationDto: PaginationDto = { page, limit };
    const result = await this.recipesService.findByAuthor(userId, paginationDto, true);
    return ApiResponseDto.success('User recipes retrieved successfully', result);
  }
  ...
  async findByAuthor(
    ...
    const paginationDto: PaginationDto = { page, limit };
    const result = await this.recipesService.findByAuthor(authorId, paginationDto, false);
    return ApiResponseDto.success('Author recipes retrieved successfully', result);
  }
  ...
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Recipe not found' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Not authorized to update this recipe' })
  async update(
    @Param('id', ParseUUIDPipe) id: string,
    ...
    const recipe = await this.recipesService.update(id, updateRecipeDto, userId, user.role);
  ...
  @ApiParam({ name: 'id', description: 'Recipe UUID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Recipe deleted successfully' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Recipe not found' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Not authorized to delete this recipe' })
  async remove(
    @Param('id', ParseUUIDPipe) id: string,
    @CurrentUserId() userId: string,
    @CurrentUser() user: any,
  ...
  async toggleFeatured(
    ...
    return ApiResponseDto.success('Recipe featured status toggled successfully', recipe);
  }
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Recipe statistics retrieved successfully' })
  async getRecipeStats(): Promise<ApiResponseDto<any>> {
    const stats = await this.recipesService.getRecipeStats();
    return ApiResponseDto.success('Recipe statistics retrieved successfully', stats);
  }
} 